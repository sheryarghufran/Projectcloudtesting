  ...
  for (var i = 0; i < 3; i++) {
    try {
      var r = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: MODEL, max_tokens: tokens || 3500, system: SYS, messages: [{ role: "user", content: prompt }] }),
      });
      if (!r.ok) continue;
      var d = await r.json();
      if (d.error) continue;
  ...
  var sendChat = useCallback(async function() {
    ...
    try {
      ...
      var r = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: MODEL, max_tokens: 2000, system: ctx2, messages: msgs }),
      });
      var d = await r.json();
      var text = (d.content || []).map(function(c) { return c.text || ""; }).join("");
      ...
      <div style={{ borderBottom: "1px solid #18181c", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ color: "#333", fontSize: 9, letterSpacing: 0.8, textTransform: "uppercase" }}>Opus 4.6 | Catalog Manager | PPC Analyst</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
          ...
          <Badge color="#f59e0b" small>Opus 4.6</Badge>
        </div>
      </div>
          ...
          <div style={{ animation: "fadeIn .3s" }}>
            {loading && (
              <div style={{ textAlign: "center", padding: 60 }}>
                ...
                <div style={{ color: "#333", fontSize: 11, animation: "pulse 2s ease infinite" }}>Powered by Claude Opus 4.6</div>
              </div>
            )}
        ...
        {tab === "chat" && (
          <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 180px)", animation: "fadeIn .3s" }}>
            <div style={{ marginBottom: 10 }}>
              ...
              <p style={{ color: "#333", fontSize: 11, margin: "2px 0 0" }}>Opus 4.6 | Catalog Manager + PPC Analyst{classify ? " | Analysis loaded" : ""}</p>
            </div>
